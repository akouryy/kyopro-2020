...
void solve(int case_id) {
  ...
  bool swapped = L < R;
  if(swapped) swap(L, R);
  {if(debug)cerr<<'#'<<__LINE__ ln<<"  swapped: "<<(swapped)ln<<"  L:       "<<(L)ln<<"  R:       "<<(R)ln;}
  int i1;
  {
    ...
    i1 = ok;
    L -= i1 * (i1 + 1) / 2;
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  i1: "<<(i1)ln<<"  L:  "<<(L)ln;}
  ...
  int dL;
  {
    ...
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  dL: "<<(dL)ln<<"  x:  "<<(x)ln;}
    assert(x >= 0);
  ...
  int dR;
  {
    ...
    while(abs(ok - ng) > 1) {
      int mid = (ok + ng) / 2;
      {if(debug)cerr<<'#'<<__LINE__ ln<<"  mid:                      "<<(mid)ln<<"  R - i1 * mid - mid * mid: "<<(R - i1 * mid - mid * mid)ln;}
    ...
    dR = ok - 1;
    int x = R - i1 * dR - dR * dR;
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  dR: "<<(dR)ln<<"  x:  "<<(x)ln;}
    assert(x >= 0);
  ...
  {if(debug)cerr<<'#'<<__LINE__ ln<<"  d:             "<<(d)ln<<"  (d / 2):       "<<((d / 2))ln<<"  ((d + 1) / 2): "<<(((d + 1) / 2))ln<<"  ll:            "<<(ll)ln<<"  rr:            "<<(rr)ln;}
