...
using VPII=vec<pair<int, int>>;using VI=vec<int>;
#line 1 "alias.hpp"//5b
...
void solve() {
  ...
  times(t, o) {
    ...
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  mp:   "<<(mp)ln<<"  msum: "<<(msum)ln;}
    int ans = 0, c = 0;
    ...
    downto(n-1, 1, i) {
      ...
      {if(debug)cerr<<'#'<<__LINE__ ln<<"  i:        "<<(i)ln<<"  c:        "<<(c)ln<<"  req:      "<<(req)ln<<"  mp:       "<<(mp)ln<<"  pq.top(): "<<(pq.top())ln;}
    }
