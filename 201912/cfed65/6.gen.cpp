...
#line 1 "util_compress.hpp"//6
TL<TN T>struct Compressed{int sz;map<T,int>zip;vec<T>unzip;Compressed(CS vec<T>&v):sz(size(v)),unzip(v){uniq(unzip);
times(sz,i)zip[unzip[i]]=i;}};TL<TN T>struct CompressedSrc{int sz;map<T,int>zip;vec<T>unzip;WI src;
CompressedSrc(CS vec<T>&v){auto c=compressed(v);sz=c.sz;zip=move(c.zip);unzip=move(c.unzip);src.resize(c.sz);
times(size(v),i)src[zip[v[i]]].PB(i);}};
#line 3001 "6.cpp"//
...
void solve() {
  ...
  times(N, i) {
    ...
    {if(debug)cerr<<"bit.data: "<<(bit.data)ln;}
  }
  ...
  {if(debug)cerr<<'#'<<__LINE__ ln<<"  x: "<<(x)ln<<"  y: "<<(y)ln;}
  mint ans = 0_m;

  times(N, i) {
    ans += mint(A[i]) * (mint(y[i]) * mint(i+1) + mint(x[i]) * mint(N-i));
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  mint(y[i]) * mint(i+1): "<<(mint(y[i]) * mint(i+1))ln<<"  mint(x[i]) * mint(N-i): "<<(mint(x[i]) * mint(N-i))ln<<"  ans:                    "<<(ans)ln;}
  }
