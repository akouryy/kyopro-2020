...
using VI=vec<int>;using VPIB=vec<pair<int, bool>>;
#line 1 "alias.hpp"//4b
...
/* </foxy.memo-area> */
  ...
  times(1<<N, h) {
    ...
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  1:  "<<(1)ln<<"  cd: "<<(cd)ln;}
    int cnt = 0;
    ...
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  2:  "<<(2)ln<<"  cd: "<<(cd)ln;}
    ...
    ans = min(ans, cnt);
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  3:   "<<(3)ln<<"  cd:  "<<(cd)ln<<"  cnt: "<<(cnt)ln;}
    ng:;
  }
