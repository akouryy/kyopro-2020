...
using VI=vec<int>;using VB=vec<bool>;using PII=pair<int, int>;using VPII=vec<pair<int, int>>;
#line 1 "alias.hpp"//4b
  ...
  times(N-3, o) {
    ...
    if(i == -1) {
      ...
      if(before >= 0 && A[before] == i) {
        int j = pq.top(); pq.pop();
        pq.push(i);
        {if(debug)cerr<<'#'<<__LINE__ ln<<"  i: "<<(i)ln<<"  j: "<<(j)ln;}
        i = j;
      }
    }
    used[i] = true;
