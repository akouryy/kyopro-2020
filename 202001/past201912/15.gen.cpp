...
using WI=vvec<int>;using VI=vec<int>;using VPIPII=vec<pair<int, pair<int, int>>>;
#line 1 "alias.hpp"//1b
...
/* </foxy.memo-area> */
  ...
  uptil(1, N*6, h) {
    ...
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  a[h].first:          "<<(a[h].first)ln<<"  alpha[a[h-1].first]: "<<(alpha[a[h-1].first])ln<<"  i0:                  "<<(i0)ln<<"  s:                   "<<(s)ln<<"  alpha[a[h].first]:   "<<(alpha[a[h].first])ln;}
  }
