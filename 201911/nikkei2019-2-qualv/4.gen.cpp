...
#line 2001 "dijkstra.hpp"//4

TL<class E>
vec<E> dijkstra(const Graph<E>& g, int s) {
  priority_queue<pair<E, int>, vec<pair<E, int>>, greater<pair<E, int>>> pq;
  vec<E> ans(g.nv(), INF);
  vec<bool> visited(g.nv());

  pq.push({E(), s});
  E e; int i;
  while(!pq.empty()) {
    tie(e, i) = pq.top(); pq.pop();
    if(visited[i]) continue;
    visited[i] = true;
    ans[i] = e;
    for(auto& t : g.edges[i]) {
      if(!visited[t.to]) {
        pq.push({e + t.weight, t.to});
      }
    }
  }
  return ans;
