...
using VS=vec<string>;using HPIII=map<pair<int, int>, int>;using VB=vec<bool>;
#line 1 "alias.hpp"//6b
...
/* </foxy.memo-area> */
  ...
  times(N, i) times(N, j) {
    ...
    if(A[i][j] == 'o') {
      ...
      if(z == 0) {
        z = 1;
        {if(debug)cerr<<'#'<<__LINE__ ln<<"  p:    "<<(p)ln<<"  q:    "<<(q)ln<<"  true: "<<(true)ln;}
      ...
      if(z == 0) {
        z = 2;
        {if(debug)cerr<<'#'<<__LINE__ ln<<"  p:     "<<(p)ln<<"  q:     "<<(q)ln<<"  false: "<<(false)ln;}
  ...
  function<pyon(int, bool, pyon)> dfs = [&](int i, bool b, pyon p) {
    ...
    vis[i] = true;
    val[i] = b;
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  i:      "<<(i)ln<<"  val[i]: "<<(val[i])ln;}
